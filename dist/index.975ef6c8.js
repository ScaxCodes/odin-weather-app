...
var _gifLogicJs = require("./gifLogic.js");
...
function parseCurrentWeather(weather) {
    ...
    const { temp_c, temp_f, condition: { text, icon } } = weather.current;
    return {
        name,
        temp_c,
        temp_f,
...
function parseHourlyWeather({ forecast: { forecastday: [currentDay] }, current: { last_updated_epoch: lastUpdatedEpoch } }) {
    // Return only hours from the current hour till midnight
    return currentDay.hour.filter((hour)=>hour.time_epoch > lastUpdatedEpoch - 3600).map((hour)=>{
        return {
...
async function displayWeather(location) {
    ...
    temp.textContent = `${weather.current.celsius}\xb0`;
    tempF.textContent = `${weather.current.fahrenheit}\xb0`;
    ...
    tempForecast.forEach((div, index)=>{
        div.textContent = `
    ${weather[`forecastDay${index + 2}`].mintemp_c}\xb0 - 
    ${weather[`forecastDay${index + 2}`].maxtemp_c}\xb0
    `;
    });
    tempFForecast.forEach((div, index)=>{
        div.textContent = `
    ${weather[`forecastDay${index + 2}`].mintemp_f}\xb0 - 
    ${weather[`forecastDay${index + 2}`].maxtemp_f}\xb0
    `;
    });
    ...
    // Display GIF
    (0, _gifLogicJs.displayGIF)(weather.current.text);
}

},{"./gifLogic.js":"cXUHj","./normalize.css":"cbx85","./styles.css":"lW6qc"}],"cXUHj":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
parcelHelpers.export(exports, "displayGIF", ()=>displayGIF);
function selectGIF(text) {
    text = text.toLowerCase();
    if (text.includes("ice") || text.includes("freez") || text.includes("blizzard")) return "//media1.giphy.com/media/KFUx0Rtz7p0HTzbJ7x/giphy.gif";
    else if (text.includes("snow")) return "//media3.giphy.com/media/26tneSGWphvmFlUju/giphy.gif";
    else if (text.includes("sun")) return "//media0.giphy.com/media/98UMeH7pPiOpM5sHDw/giphy.gif";
    else if (text.includes("rain")) return "//media1.giphy.com/media/Mgq7EMQUrhcvC/giphy.gif";
    else if (text.includes("cloud") || text.includes("overcast") || text.includes("mist")) return "//media0.giphy.com/media/gk3s6G7AdUNkey0YpE/giphy.gif";
    else if (text.includes("fog")) return "//media3.giphy.com/media/ZWRCWdUymIGNW/giphy.gif";
    else return 0;
}
...
},{}],"cbx85":[function() {},{}],"lW6qc":[function() {},{}]},["farZc","8lqZg"], "8lqZg", "parcelRequired5ef")
